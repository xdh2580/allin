...
# 获取一个文件目录下的所有test_failures_suite.html报告，注意path中存放报告的规范，最多搜索到二级子目录
...
def getreport(path):
    ...
    for i in list_dir:
        if i == "cts" or "vts" or "sts" or "gts" or "gsi":
                    ...
                    if j[:3] == "202":
                        ...
                        try:
                            for q in os.listdir(sub_list2):
                                if q == "test_result_failures_suite.html":
...
# 从一系列报告信息（字典形式）中获取信息并写入到xlsx表格
# all_info：报告信息字典的列表
def write_xl(all_info):
    workbook = load_workbook(filename="case.xlsx")
    sheet1 = workbook["case汇总"]
    row0 = ["plan", "tool", "case_pass", "case_fail", "modules_done", "modules_total", "finger_print"]
    sheet1.append(row0)
    data = []
    for info in all_info:
        ...
        row = [plan, build, case_pass, case_fail, modules_done, modules_total, finger_print]
        data.append(row)
    for r in data:
        sheet1.append(r)
    workbook.save(filename="汇总.xlsx")
